...
((window, factory) => {
  if (typeof define == 'function' && define.amd) {
    ...
    window.ChromeTabs = factory(window, require('draggabilly'))
  }
  ...
  class ChromeTabs {
    ...
    init(el) {
      ...
      this.viewContainer = document.getElementById("etabs-views")
    }
    ...
    addTab(tabProperties, { animate = true, background = false } = {}) {
      ...
      this.layoutTabs()
      this.setupDraggabilly()
      let webview = this.initWebview(tabProperties)
      tabs.push({ webview: webview, bag: tabProperties.bag, tabEl: tabEl })
      //webview.classList.add('visible')
      if (!background) this.setCurrentTab(tabEl)
    }
    initWebview(tabProperties) {
      ...
      webview.addEventListener("did-finish-load", tabWebviewDidFinishLoadHandler.bind(this), false);
      ...
      webview.addEventListener("dom-ready", tabWebviewDomReadyHandler.bind(this), false);
      webview.setAttribute("src", tabProperties.src);
      webview.addEventListener('ipc-message', (event) => {
        //TODO: 需添加权限验证
        // console.log("event:", event); // detail-window-start-download
        console.log("event.channel", event.channel);// detail-window-start-download
        console.log("event.args", event.args); // arg[0]: "CAPJ", arg[1]:"DLGD2020112600B" 
        if (event.channel == "detail-window-start-download") {
          ipcRenderer.send("detail-window-start-download-file", event.args[0]);
        }// (event, fileid, typeid, title, count, filetype) 
      });

      //webview.classList.add(this.tabGroup.options.viewClass);
      ...
      this.viewContainer.appendChild(webview);
      return webview;
    }
    ...
    setCurrentTab(tabEl) {
      ...
      if (activeTabEl) {
        ...
        tab = this.getTabByEl(activeTabEl)
        if (tab && tab.webview) tab.webview.classList.remove('visible');
      }
      tabEl.setAttribute('active', '')
      tab = this.getTabByEl(tabEl)
      if (tab && tab.webview) tab.webview.classList.add('visible');
    ...
    removeTab(tabEl) {
      ...
      var tab = this.getTabByEl(tabEl)
      if (tab && tab.webview) {
        this.viewContainer.removeChild(tab.webview);
